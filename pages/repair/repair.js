...
Page({
  data: {
    ...
    uploadVoucherPhotos: [],
    uploadFailPhotos: [],
    uploadUrl: api.baseUrl + '/upload/uploadVoucherImg',
  ...
  subscribe: function(e) {
    ...
    let photosStr = ''
    let voucherImages = ''
    for (let i = 0; i < this.data.uploadVoucherPhotos.length; i++){
        photosStr += this.data.uploadVoucherPhotos[i].url + ','
    }
    for (let j = 0; j < this.data.uploadFailPhotos.length; j++){
        voucherImages += this.data.uploadFailPhotos[j].url + ','
    }
    ...
    if (this.data.failList.length === 0 || remark === '' || remark === undefined) {
      wx.showToast({
        title: '故障原因不能空',
        icon: 'none'
      })
    ...
    for (let i = 0; i < this.data.failList.length; i++) {
      if (this.data.failList[i].checked) {
